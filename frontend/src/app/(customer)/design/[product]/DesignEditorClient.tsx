...
export default function DesignEditorClient({ product }: { product?: string }) {
  const hookParams = useParams<{ product: string }>();
  const resolvedProduct = product || hookParams?.product;

  return <Designer productId={resolvedProduct} />;
}
