...
export const freepikService = {
  ...
  removeBackground: async (
    ...
    if (!normalizedImageUrl) {
      throw new Error('An image URL is required to remove the background.');
    }

    // Always call the Laravel backend. Calling Magnific directly from the
    // browser exposes the API key and is blocked by the provider's CORS rules.
